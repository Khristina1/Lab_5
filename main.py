    ...
    for i in range(len(matrix)):
        for j in range(len(matrix)):
            ...
            if result != 0 and j not in deletedRows:
                ...
                print("Стретегія А" + str(i + 1), " є домінуючою над стратегією А" + str(j + 1), "тому забирається рядок", j + 1)
    ...
    for i in range(len(transposedMatrix)):
        for j in range(len(transposedMatrix)):
            ...
            if result != 0 and j not in deletedColumns:
                ...
                print("Стретегія В" + str(i + 1), "є домінуючою над стратегією В" + str(j + 1), "тому забирається стовпчик", j + 1)
...
print(Fore.BLUE + "Вхідні дані завдання", Style.RESET_ALL)
print(get_matrix_table(matrix))

print(Fore.BLUE + "Перевірка матриці на наявність сідлової точки", Style.RESET_ALL)
x = PrettyTable()
fields = ['']
for i in range(len(matrix)):
    fields.append("B" + str(i + 1))
...
for i in range(len(matrix)):
    minA[i] = min(matrix[i])
    x.add_row(['A' + str(i + 1)] + matrix[i] + [Fore.GREEN + str(minA[i]) + Style.RESET_ALL])
    for j in range(len(matrix[i])):
        if j not in maxB or maxB[j] < matrix[i][j]:
            maxB[j] = matrix[i][j]

x.add_row(['b = max(Bi)' + Fore.CYAN] + [maxB[element] for element in maxB] + ['' + Style.RESET_ALL])
print(x)
...
if maxFromMatrixA == minFromMatrixB:
    ...
    print("Сідлова точка відсутня, тому що a != b")
    print("Ціна гри знаходиться в таких межах:", Fore.RED, maxFromMatrixA, "<= y <=", minFromMatrixB, Style.RESET_ALL)


print(Fore.BLUE, "\nПеревірка матриці на домінуючі рядки і домінуючі стовпці:", Style.RESET_ALL)
print(Fore.BLUE + "З позиції виграшу гравця А", Style.RESET_ALL)
matrixAfterExcludingRows = check_dominant_rows(matrix)
print(Fore.RED + "Після перевірки домінуючих рядків матриця виглядає так: ", Style.RESET_ALL)
print(get_matrix_table(matrixAfterExcludingRows))

print(Fore.BLUE + "З позиції програшу гравця В", Style.RESET_ALL)
matrixAfterExcludingColumns = check_dominant_columns(matrixAfterExcludingRows)
print(Fore.RED + "Після перевірки домінуючих стовпчиків матриця виглядає так: ", Style.RESET_ALL)
...
print(Fore.RED + "Знайти мінімум функції F(x) при обмеженнях (для гравця B)", Style.RESET_ALL)
...
print(Fore.RED + "Знайти мінімум функції Z(y) при обмеженнях (для гравця A)", Style.RESET_ALL)
...
print(Fore.BLUE + "\nРозв'язання  задачі лінійного програмування симплексним методом", Style.RESET_ALL)
print(Fore.BLUE + "Визначимо максимальне значення цільової функції", Fore.RED, mainCondition + '--> max', Fore.BLUE + "при настуних умовах-обмеженнях:", Style.RESET_ALL)
print(conditions)
print(Fore.BLUE + "Після переведення в канонічну форму переходимо до основно алгоритму симплекс-методом", Style.RESET_ALL)
...
print("\nЦіна гри дорівнює g = 1/F(x)")
print(Fore.RED + "g = 1/(" + str(xResult), ") =", str(1/xResult), Style.RESET_ALL)
